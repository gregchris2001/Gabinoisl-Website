...
const Product = () => {
    const { productData, cartData, addCartData, changeCartProductQuantity, removeCartData, changeRecentlyViewed } = useContext(ProductContext);
    ...
    const productFromContext = productData?.find(product => product.title === item);
    ...
    useEffect(() => {
        ...
        const productInCart = cartData?.find(product => product.id === product.id); 
        if (productInCart) {
            setInCart(true);
